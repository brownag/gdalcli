...
#' @title simplify-coverage: Simplify shared boundaries of a polygonal vector dataset
#' @description
#' `gdal vector simplify-coverage` simplifies boundaries of a polygonal dataset, such that shared boundaries are preserved without introducing gaps or overlaps between features. Gaps or overlaps already present in the input dataset will not be corrected. This requires loading the entire dataset into memory at once. If preservation of shared boundaries is not needed, gdal_vector_simplify provides an alternative that can process geometries in a streaming manner. Simplification is performed using the Visvalingam-Whyatt algorithm. This command can also be used as a step of gdal_vector_pipeline.
#' 
#' See \url{https://gdal.org/en/release-3.13/programs/gdal_vector_simplify-coverage.html} for detailed GDAL documentation.
...
#' @param preserve_boundary Whether the exterior boundary should be preserved. (Logical)
...
#' job <- gdal_vector_simplify_coverage(input = "in.gpkg", output = "out.gpkg", 
#'     overwrite = TRUE, tolerance = 1)
#' \dontrun{
#'   result <- gdal_job_run(job)
#' }
#' @export
gdal_vector_simplify_coverage <- function(input,
  input_format = NULL,
  input_layer = NULL,
  output,
  output_format = NULL,
  ...
  if (!missing(preserve_boundary)) new_args[["preserve_boundary"]] <- preserve_boundary
  ...
  if (!missing(input) && inherits(input, 'gdal_job')) {
    ...
    return(extend_gdal_pipeline(piped_job, c("vector", "simplify-coverage"), new_args))
  }


  merged_args <- .merge_alias_parameters(list(...), new_args)
  .update_intent_mapping <- NULL
  .update_intent <- infer_update_intent("gdal_vector_simplify_coverage", merged_args, .update_intent_mapping)
  ...
  .arg_mapping <- list(
    ...
    tolerance = list(min_count = 0, max_count = 1),
    ...
    preserve_boundary = list(min_count = 0, max_count = 1)
  )

  new_gdal_job(command_path = c("vector", "simplify-coverage"), arguments = merged_args, arg_mapping = .arg_mapping, update_intent = .update_intent)
}
