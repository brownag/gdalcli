...
#' @description
#' `gdal vector geom set-type` can be used to modify the geometry type of geometry fields at the layer and/or feature level. It can also be used as a step of gdal_vector_pipeline. The following groups of options can be combined together: - --multi / --single - --linear / --curve - --dim
#' 
#' See \url{https://gdal.org/en/release-3.11/programs/gdal_vector_geom_set-type.html} for detailed GDAL documentation.
...
gdal_vector_geom_set_type <- function(input,
  ...
  new_gdal_job(command_path = c("vector", "geom", "set-type"), arguments = merged_args, arg_mapping = .arg_mapping, update_intent = .update_intent)
}
