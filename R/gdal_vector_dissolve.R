...
#' @title dissolve: Dissolves multipart features
#' @description
#' `gdal vector dissolve` performs a union operation on the elements of each feature's geometry. This has the following effects: - Duplicate vertices are eliminated. - Nodes are added where input linework intersects. - Polygons that overlap are "dissolved" into a single feature. For linear geometries, the union step is followed by a line-merging step, where lines are merged at points that form an endpoint of exactly two lines. To dissolve the geometries of multiple features together, first combine them into single features with gdal_vector_combine. ``dissolve`` can be used as a step of gdal_vector_pipeline.
#' 
#' See \url{https://gdal.org/en/release-3.13/programs/gdal_vector_dissolve.html} for detailed GDAL documentation.
...
#' @examples
#' \dontrun{
#' # TODO: No examples available for gdal_vector_dissolve.
#' # See GDAL documentation at:
#' https://gdal.org/en/release-3.13/programs/gdal_vector_dissolve.html
#' job <- gdal_vector_dissolve()
#' # gdal_job_run(job)
#' }
#' @export
gdal_vector_dissolve <- function(input,
  input_format = NULL,
  input_layer = NULL,
  output,
  output_format = NULL,
  ...
  if (!missing(input) && inherits(input, 'gdal_job')) {
    ...
    return(extend_gdal_pipeline(piped_job, c("vector", "dissolve"), new_args))
  }


  merged_args <- .merge_alias_parameters(list(...), new_args)
  .update_intent_mapping <- NULL
  .update_intent <- infer_update_intent("gdal_vector_dissolve", merged_args, .update_intent_mapping)
  ...
  new_gdal_job(command_path = c("vector", "dissolve"), arguments = merged_args, arg_mapping = .arg_mapping, update_intent = .update_intent)
}
