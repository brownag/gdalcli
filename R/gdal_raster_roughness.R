...
#' @title roughness: Generate a roughness map
#' @description
#' `gdal raster roughness` generates a single-band raster with values computed from the elevation. Roughness is the largest inter-cell difference of a central pixel and its surrounding cell, as defined in Wilson et al. (2007, Marine Geodesy 30:3-35). This subcommand is also available as a potential step of gdal_raster_pipeline The roughness value -9999 is used as the nodata value. A nodata value in the target dataset will also be emitted if at least one pixel set to the nodata value is found in the 3x3 window centered around each source pixel. By default, the algorithm will compute values at image edges or if a nodata value is found in the 3x3 window, by interpolating missing values, unless --no-edges is specified, in which case a 1-pixel border around the image will be set with the nodata value.
#' 
#' See \url{https://gdal.org/en/release-3.13/programs/gdal_raster_roughness.html} for detailed GDAL documentation.
...
gdal_raster_roughness <- function(input,
  ...
  if (!missing(input) && inherits(input, 'gdal_job')) {
    ...
    return(extend_gdal_pipeline(piped_job, c("raster", "roughness"), new_args))
  }
  ...
  new_gdal_job(command_path = c("raster", "roughness"), arguments = merged_args, arg_mapping = .arg_mapping, update_intent = .update_intent)
}
