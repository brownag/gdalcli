...
#' @description
#' `gdal vector make-valid` ensures that geometries are valid regarding the rules of the Simple Features specification. It runs the :cppOGRGeometry::MakeValid operation, followed by :cppOGRGeometryFactory::removeLowerDimensionSubGeoms (unless --keep-lower-dim is set) It can also be used as a step of gdal_vector_pipeline.
#' 
#' See \url{https://gdal.org/en/release-3.13/programs/gdal_vector_make-valid.html} for detailed GDAL documentation.
...
gdal_vector_make_valid <- function(input,
  ...
  .update_intent <- infer_update_intent("gdal_vector_make_valid", merged_args, .update_intent_mapping)
  ...
  .arg_mapping <- list(
    ...
    keep_lower_dim = list(min_count = 0, max_count = 1)
  )

  new_gdal_job(command_path = c("vector", "make-valid"), arguments = merged_args, arg_mapping = .arg_mapping, update_intent = .update_intent)
}
