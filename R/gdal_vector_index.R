...
#' @description
#' `gdal vector index` creates a vector dataset with a record for each input vector file, an attribute containing the filename (suffixed with comma and the index of the layer within the dataset) and a polygon geometry outlining the (rectangular) extent. This output is suitable for use with `MapServer <http://mapserver.org/>`__ as a vector tileindex
#' 
#' See \url{https://gdal.org/en/release-3.13/programs/gdal_vector_index.html} for detailed GDAL documentation.
...
#' @param source_layer_name Add layer of specified name from each source file in the tile index (Character vector)
#' @param source_layer_index Add layer of specified index (0-based) from each source file in the tile index (Integer vector)
...
gdal_vector_index <- function(input,
  ...
  .update_intent <- infer_update_intent("gdal_vector_index", merged_args, .update_intent_mapping)
  ...
  new_gdal_job(command_path = c("vector", "index"), arguments = merged_args, arg_mapping = .arg_mapping, update_intent = .update_intent)
}
