...
#' @description
#' `gdal vector segmentize` ensures segments of LineStrings or rings of Polygons are not longer than the specified max length, by inserting intermediate points. It runs the :cppOGRGeometry::segmentize operation. It can also be used as a step of gdal_vector_pipeline.
#' 
#' See \url{https://gdal.org/en/release-3.13/programs/gdal_vector_segmentize.html} for detailed GDAL documentation.
...
#' job <- gdal_vector_segmentize(input = "in.gpkg", output = "out.gpkg", overwrite = TRUE, 
#'     max_length = 1000)
#' \dontrun{
#'   result <- gdal_job_run(job)
#' }
#' @export
gdal_vector_segmentize <- function(input,
  input_format = NULL,
  input_layer = NULL,
  output,
  output_format = NULL,
  ...
  if (!missing(input) && inherits(input, 'gdal_job')) {
    ...
    return(extend_gdal_pipeline(piped_job, c("vector", "segmentize"), new_args))
  }


  merged_args <- .merge_alias_parameters(list(...), new_args)
  .update_intent_mapping <- NULL
  .update_intent <- infer_update_intent("gdal_vector_segmentize", merged_args, .update_intent_mapping)
  ...
  new_gdal_job(command_path = c("vector", "segmentize"), arguments = merged_args, arg_mapping = .arg_mapping, update_intent = .update_intent)
}
