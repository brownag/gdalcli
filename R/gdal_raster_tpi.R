...
gdal_raster_tpi <- function(input,
  ...
  if (!missing(input) && inherits(input, 'gdal_job')) {
    ...
    return(extend_gdal_pipeline(piped_job, c("raster", "tpi"), new_args))
  }


  merged_args <- .merge_alias_parameters(list(...), new_args)
  .update_intent_mapping <- NULL
  .update_intent <- infer_update_intent("gdal_raster_tpi", merged_args, .update_intent_mapping)
  ...
  new_gdal_job(command_path = c("raster", "tpi"), arguments = merged_args, arg_mapping = .arg_mapping, update_intent = .update_intent)
}
