...
#' @title sort: Spatially order the features in a layer
#' @description
#' `gdal vector sort` sorts the features in a vector dataset according to the position of its geometries. The sorting method can be specified via --method, with Hilbert and Sort-Tile-Recursive (STRTree) algorithms available. Features that have null or empty geometries will be placed at the end of the sorted dataset. This command can also be used as a step of gdal_vector_pipeline.
#' 
#' See \url{https://gdal.org/en/release-3.13/programs/gdal_vector_sort.html} for detailed GDAL documentation.
...
#' @param geometry_field Name of geometry field to use in sort
#' @param method Geometry sorting algorithm. Choices: "hilbert", "strtree" (Default: `hilbert`)
#' @param use_tempfile Write features to a temporary file to avoid reading the entire input dataset into memory (Logical)
...
#' job <- gdal_vector_sort(input = "in.gpkg", output = "out.shp", method = "hilbert")
#' \dontrun{
#'   result <- gdal_job_run(job)
#' }
#' @export
gdal_vector_sort <- function(input,
  input_format = NULL,
  input_layer = NULL,
  output,
  output_format = NULL,
  ...
  if (!missing(use_tempfile)) new_args[["use_tempfile"]] <- use_tempfile
  ...
  if (!missing(input) && inherits(input, 'gdal_job')) {
    ...
    return(extend_gdal_pipeline(piped_job, c("vector", "sort"), new_args))
  }


  merged_args <- .merge_alias_parameters(list(...), new_args)
  .update_intent_mapping <- NULL
  .update_intent <- infer_update_intent("gdal_vector_sort", merged_args, .update_intent_mapping)
  ...
  .arg_mapping <- list(
    ...
    use_tempfile = list(min_count = 0, max_count = 1)
  )

  new_gdal_job(command_path = c("vector", "sort"), arguments = merged_args, arg_mapping = .arg_mapping, update_intent = .update_intent)
}
