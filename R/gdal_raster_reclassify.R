...
#' @description
#' `gdal raster reclassify` reclassifies values in an input dataset. A file (or string) specifies the mapping of input pixel values or ranges to output files. Ranges of values can be specified using brackets:
#' 
#' See \url{https://gdal.org/en/release-3.13/programs/gdal_raster_reclassify.html} for detailed GDAL documentation.
...
#' https://gdal.org/en/release-3.13/programs/gdal_raster_reclassify.html
...
gdal_raster_reclassify <- function(input,
  ...
  .update_intent <- infer_update_intent("gdal_raster_reclassify", merged_args, .update_intent_mapping)
  ...
  .arg_mapping <- list(
    ...
    mapping = list(min_count = 0, max_count = 1),
  ...
  new_gdal_job(command_path = c("raster", "reclassify"), arguments = merged_args, arg_mapping = .arg_mapping, update_intent = .update_intent)
}
