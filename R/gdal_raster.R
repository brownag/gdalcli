...
#' 
#' See \url{https://gdal.org/en/release-3.13/programs/gdal_raster.html} for detailed GDAL documentation.
...
#' @examples
#' \dontrun{
#' # TODO: No examples available for gdal_raster.
#' # See GDAL documentation at:
#' https://gdal.org/en/release-3.13/programs/gdal_raster.html
#' job <- gdal_raster()
#' # gdal_job_run(job)
#' }
#' @export
gdal_raster <- function(drivers = FALSE,
  ...
  .update_intent <- infer_update_intent("gdal_raster", merged_args, .update_intent_mapping)

  .arg_mapping <- list(
    drivers = list(min_count = 0, max_count = 1)
  )

  new_gdal_job(command_path = c("raster"), arguments = merged_args, arg_mapping = .arg_mapping, update_intent = .update_intent)
}
