...
#' 
#' See \url{https://gdal.org/en/release-3.13/programs/gdal_mdim.html} for detailed GDAL documentation.
...
#' https://gdal.org/en/release-3.13/programs/gdal_mdim.html
#' job <- gdal_mdim()
#' # gdal_job_run(job)
#' }
#' @export
gdal_mdim <- function(drivers = FALSE,
  ...
  .update_intent <- infer_update_intent("gdal_mdim", merged_args, .update_intent_mapping)

  .arg_mapping <- list(
    drivers = list(min_count = 0, max_count = 1)
  )

  new_gdal_job(command_path = c("mdim"), arguments = merged_args, arg_mapping = .arg_mapping, update_intent = .update_intent)
}
