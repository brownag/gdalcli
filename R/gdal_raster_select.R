...
#' @description
#' `gdal raster select` can be used to select and re-order a subset of raster bands from a raster dataset. This subcommand is also available as a potential step of gdal_raster_pipeline
#' 
#' See \url{https://gdal.org/en/release-3.13/programs/gdal_raster_select.html} for detailed GDAL documentation.
...
#' @param band Band(s) (1-based index, 'mask', 'mask:<band>' or color interpretation such as 'red') (Character vector) (required). `1` to `2147483647` value(s)
...
#' @param exclude Exclude specified bands (Logical)
...
gdal_raster_select <- function(input,
  ...
  .update_intent <- infer_update_intent("gdal_raster_select", merged_args, .update_intent_mapping)
  ...
  new_gdal_job(command_path = c("raster", "select"), arguments = merged_args, arg_mapping = .arg_mapping, update_intent = .update_intent)
}
