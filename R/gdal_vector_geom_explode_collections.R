...
#' 
#' See \url{https://gdal.org/en/release-3.11/programs/gdal_vector_geom_explode-collections.html} for detailed GDAL documentation.
...
#' job <- gdal_vector_geom_explode_collections(input = "in.gpkg", output = "points.shp", 
...
gdal_vector_geom_explode_collections <- function(input,
  ...
  .update_intent <- infer_update_intent("gdal_vector_geom_explode_collections", merged_args, .update_intent_mapping)
  ...
  new_gdal_job(command_path = c("vector", "geom", "explode-collections"), arguments = merged_args, arg_mapping = .arg_mapping, update_intent = .update_intent)
}
