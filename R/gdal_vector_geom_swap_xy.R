...
#' @description
#' `gdal vector geom swap-xy` swaps the X and Y coordinates of geometries, typically to work around axis order issues of coordinate reference systems. It can also be used as a step of gdal_vector_pipeline.
#' 
#' See \url{https://gdal.org/en/release-3.11/programs/gdal_vector_geom_swap-xy.html} for detailed GDAL documentation.
...
gdal_vector_geom_swap_xy <- function(input,
  ...
  .update_intent <- infer_update_intent("gdal_vector_geom_swap_xy", merged_args, .update_intent_mapping)
  ...
  new_gdal_job(command_path = c("vector", "geom", "swap-xy"), arguments = merged_args, arg_mapping = .arg_mapping, update_intent = .update_intent)
}
