...
#' @description
#' `gdal vector set-field-type` can be used to modify the field type of a vector dataset: ``set-field-type`` can also be used as a step of gdal_vector_pipeline.
#' 
#' See \url{https://gdal.org/en/release-3.13/programs/gdal_vector_set-field-type.html} for detailed GDAL documentation.
#' @param input Input vector datasets (required). Exactly `1` value(s). Can also be a [gdal_job] object to extend a pipeline
#' @param input_field_type Source field type or subtype (required)
...
gdal_vector_set_field_type <- function(input,
  ...
  if (!missing(input_field_type)) new_args[["input_field_type"]] <- input_field_type
  ...
  .update_intent <- infer_update_intent("gdal_vector_set_field_type", merged_args, .update_intent_mapping)

  .arg_mapping <- list(
    input = list(min_count = 1, max_count = 1),
    input_field_type = list(min_count = 0, max_count = 1),
  ...
  new_gdal_job(command_path = c("vector", "set-field-type"), arguments = merged_args, arg_mapping = .arg_mapping, update_intent = .update_intent)
}
