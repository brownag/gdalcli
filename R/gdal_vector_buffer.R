...
#' 
#' See \url{https://gdal.org/en/release-3.13/programs/gdal_vector_buffer.html} for detailed GDAL documentation.
...
#' job <- gdal_vector_buffer(input = "in.gpkg", output = "out.gpkg", overwrite = TRUE, 
#'     distance = 1000)
#' \dontrun{
#'   result <- gdal_job_run(job)
#' }
#' @export
gdal_vector_buffer <- function(input,
  input_format = NULL,
  input_layer = NULL,
  output,
  output_format = NULL,
  ...
  if (!missing(input) && inherits(input, 'gdal_job')) {
    ...
    return(extend_gdal_pipeline(piped_job, c("vector", "buffer"), new_args))
  }


  merged_args <- .merge_alias_parameters(list(...), new_args)
  .update_intent_mapping <- NULL
  .update_intent <- infer_update_intent("gdal_vector_buffer", merged_args, .update_intent_mapping)
  ...
  new_gdal_job(command_path = c("vector", "buffer"), arguments = merged_args, arg_mapping = .arg_mapping, update_intent = .update_intent)
}
