...
#' @description
#' `gdal raster update` can be used to update an existing target raster dataset with the pixel values of the source raster, doing reprojection if the source and target datasets do not have the same coordinate reference systems. The extent, size, resolution or coordinate reference system of the target dataset are not modified by this operation. Overviews are updated by this command (using gdal_raster_overview_refresh), unless --no-update-overviews is specified.
#' 
#' See \url{https://gdal.org/en/release-3.12/programs/gdal_raster_update.html} for detailed GDAL documentation.
...
#' https://gdal.org/en/release-3.12/programs/gdal_raster_update.html
