...
#' @title filter: Filter a vector dataset
#' @description
#' `gdal vector filter` can be used to filter a vector dataset from their spatial extent or a SQL WHERE clause. ``filter`` can also be used as a step of gdal_vector_pipeline.
#' 
#' See \url{https://gdal.org/en/release-3.13/programs/gdal_vector_filter.html} for detailed GDAL documentation.
...
#' @param update_extent Update layer extent to take into account the filter (Logical)
...
gdal_vector_filter <- function(input,
  ...
  if (!missing(update_extent)) new_args[["update_extent"]] <- update_extent
  ...
  if (!missing(input) && inherits(input, 'gdal_job')) {
    ...
    return(extend_gdal_pipeline(piped_job, c("vector", "filter"), new_args))
  }
  ...
  .arg_mapping <- list(
    ...
    update_extent = list(min_count = 0, max_count = 1)
  )

  new_gdal_job(command_path = c("vector", "filter"), arguments = merged_args, arg_mapping = .arg_mapping, update_intent = .update_intent)
}
