...
#' @title Filter a vector dataset.
#' @description
#' Auto-generated GDAL CLI wrapper.
#' Filter a vector dataset.
#' 
#' See \url{https://gdal.org/en/stable/programs/gdal_vector_filter.html} for detailed GDAL documentation.
...
#' job <- gdal_vector_filter(input = "data.tif")
#' #
#' # Inspect the job (optional)
#' # print(job)

#' @export
gdal_vector_filter <- function(job = NULL,
  input,
  input_format = NULL,
  input_layer = NULL,
  output = NULL,
  ...
  job_input <- handle_job_input(job, new_args, c("vector", "filter"))
  if (job_input$should_extend) {
    # Extend pipeline from existing job
    return(extend_gdal_pipeline(job_input$job, c("vector", "filter"), new_args))
  } else {
    # Create new job with merged arguments
    merged_args <- job_input$merged_args
  }

  new_gdal_job(command_path = c("vector", "filter"), arguments = merged_args)
}
