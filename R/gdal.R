...
#' 
#' See \url{https://gdal.org/en/release-3.13/programs/gdal.html} for detailed GDAL documentation.
...
#' https://gdal.org/en/release-3.13/programs/gdal.html
#' job <- gdal()
#' # gdal_job_run(job)
#' }
#' @export
gdal <- function(x = NULL,
  ...
  if (!is.null(x)) {
    ...
    if (inherits(x, 'gdal_job')) {
      ...
      return(new_gdal_job(command_path = x$command_path, arguments = merged_args, update_intent = x$update_intent))
    }
    ...
    if (is.character(x) && length(x) == 1 && !grepl('\\s', x)) {
      ...
      return(new_gdal_job(command_path = c('info'), arguments = merged_args, update_intent = "SAFE"))
    }
    ...
    if (is.character(x) && length(x) == 1 && grepl('!', x)) {
      ...
      return(new_gdal_job(command_path = c('pipeline'), arguments = merged_args, update_intent = "SAFE"))
    }
    ...
    if (is.character(x) && length(x) > 1) {
      ...
      return(new_gdal_job(command_path = command_path, arguments = merged_args, update_intent = "SAFE"))
    }
  ...
  new_gdal_job(command_path = c("gdal"), arguments = merged_args, arg_mapping = .arg_mapping, update_intent = .update_intent)
}
