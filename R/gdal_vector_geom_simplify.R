...
#' job <- gdal_vector_geom_simplify(input = "in.gpkg", output = "out.gpkg", 
#'     overwrite = TRUE, tolerance = 1000)
#' \dontrun{
...
gdal_vector_geom_simplify <- function(input,
  ...
  if (!missing(input) && inherits(input, 'gdal_job')) {
    ...
    return(extend_gdal_pipeline(piped_job, c("vector", "geom", "simplify"), new_args))
  }
  ...
  new_gdal_job(command_path = c("vector", "geom", "simplify"), arguments = merged_args, arg_mapping = .arg_mapping, update_intent = .update_intent)
}
