...
#' @description
#' `gdal vector simplify` simplifies geometries, ensuring that the result is a valid geometry having the same dimension and number of components as the input. The simplification uses a maximum distance difference algorithm similar to the one used in the Douglas-Peucker algorithm. This done by the :cppOGRGeometry::SimplifyPreserveTopology method which preserves the topology per feature, but not for a whole layer. Thus gaps or overlaps between geometries that were initially contiguous may happen. To perform simplification that preserves shared boundaries between geometries, see gdal_vector_simplify_coverage. This command can also be used as a step of gdal_vector_pipeline.
#' 
#' See \url{https://gdal.org/en/release-3.13/programs/gdal_vector_simplify.html} for detailed GDAL documentation.
...
#' job <- gdal_vector_simplify(input = "in.gpkg", output = "out.gpkg", overwrite = TRUE, 
#'     tolerance = 1000)
#' \dontrun{
#'   result <- gdal_job_run(job)
#' }
#' @export
gdal_vector_simplify <- function(input,
  input_format = NULL,
  input_layer = NULL,
  output,
  output_format = NULL,
  ...
  if (!missing(input) && inherits(input, 'gdal_job')) {
    ...
    return(extend_gdal_pipeline(piped_job, c("vector", "simplify"), new_args))
  }


  merged_args <- .merge_alias_parameters(list(...), new_args)
  .update_intent_mapping <- NULL
  .update_intent <- infer_update_intent("gdal_vector_simplify", merged_args, .update_intent_mapping)
  ...
  new_gdal_job(command_path = c("vector", "simplify"), arguments = merged_args, arg_mapping = .arg_mapping, update_intent = .update_intent)
}
