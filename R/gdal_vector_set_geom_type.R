...
#' @title set-geom-type: Modify the geometry type of a vector dataset
#' @description
#' `gdal vector set-geom-type` can be used to modify the geometry type of geometry fields at the layer and/or feature level. It can also be used as a step of gdal_vector_pipeline. The following groups of options can be combined together: - --multi / --single - --linear / --curve - --dim
#' 
#' See \url{https://gdal.org/en/release-3.13/programs/gdal_vector_set-geom-type.html} for detailed GDAL documentation.
...
#' job <- gdal_vector_set_geom_type(input = "in.shp", output = "out.gpkg", overwrite = TRUE, 
#'     geometry_type = "MULTIPOLYGON")
#' \dontrun{
#'   result <- gdal_job_run(job)
#' }
#' @export
gdal_vector_set_geom_type <- function(input,
  input_format = NULL,
  input_layer = NULL,
  output,
  output_format = NULL,
  ...
  if (!missing(input) && inherits(input, 'gdal_job')) {
    ...
    return(extend_gdal_pipeline(piped_job, c("vector", "set-geom-type"), new_args))
  }


  merged_args <- .merge_alias_parameters(list(...), new_args)
  .update_intent_mapping <- NULL
  .update_intent <- infer_update_intent("gdal_vector_set_geom_type", merged_args, .update_intent_mapping)
  ...
  new_gdal_job(command_path = c("vector", "set-geom-type"), arguments = merged_args, arg_mapping = .arg_mapping, update_intent = .update_intent)
}
