...
#' @description
#' `gdal vector explode-collections` produces one feature for each geometry in any kind of geometry collection. For example if a feature contains a geometry ``MULTIPOINT(1 2,3 4)``, two corresponding target features will be generated: one with a geometry ``POINT(1 2)`` and another one with geometry ``POINT(3 4)``. Note that collections are not recursively exploded, i.e. ``GEOMETRYCOLLECTION(GEOMETRYCOLLECTION(POINT (1 2),POINT(3 4))`` will be exploded as ``GEOMETRYCOLLECTION(POINT (1 2),POINT(3 4))``. It can also be used as a step of gdal_vector_pipeline.
#' 
#' See \url{https://gdal.org/en/release-3.13/programs/gdal_vector_explode-collections.html} for detailed GDAL documentation.
...
#' job <- gdal_vector_explode_collections(input = "in.gpkg", output = "points.shp", 
#'     skip_on_type_mismatch = TRUE, overwrite = TRUE, geometry_type = "POINT")
#' \dontrun{
#'   result <- gdal_job_run(job)
#' }
#' @export
gdal_vector_explode_collections <- function(input,
  input_format = NULL,
  input_layer = NULL,
  output,
  output_format = NULL,
  ...
  if (!missing(input) && inherits(input, 'gdal_job')) {
    ...
    return(extend_gdal_pipeline(piped_job, c("vector", "explode-collections"), new_args))
  }


  merged_args <- .merge_alias_parameters(list(...), new_args)
  .update_intent_mapping <- NULL
  .update_intent <- infer_update_intent("gdal_vector_explode_collections", merged_args, .update_intent_mapping)
  ...
  new_gdal_job(command_path = c("vector", "explode-collections"), arguments = merged_args, arg_mapping = .arg_mapping, update_intent = .update_intent)
}
