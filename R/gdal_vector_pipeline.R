...
#' @title Process a vector dataset applying several steps
#' @description
...
#' 
#' See \url{https://gdal.org/en/release-3.13/programs/gdal_vector_pipeline.html} for detailed GDAL documentation.
...
gdal_vector_pipeline <- function(jobs = NULL,
  ...
  if (!missing(output_open_option)) args[["output_open_option"]] <- output_open_option
  if (!missing(output_layer)) args[["output_layer"]] <- output_layer
  if (!missing(open_option)) args[["open_option"]] <- open_option
  if (!missing(quiet)) args[["quiet"]] <- quiet
  ...
  if (!missing(upsert)) args[["upsert"]] <- upsert
