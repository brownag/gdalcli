...
#' @description
#' `gdal raster footprint` computes the footprint of a raster file, taking into account nodata values (or more generally the mask band attached to the raster bands), and generating polygons/multipolygons corresponding to areas where pixels are valid, and write to an output vector file. Since GDAL 3.12, this algorithm can be part of a gdal_pipeline. The gdal raster clean-collar utility may be run as a pre-processing step to generate proper mask bands.
#' 
#' See \url{https://gdal.org/en/release-3.13/programs/gdal_raster_footprint.html} for detailed GDAL documentation.
...
gdal_raster_footprint <- function(input,
  ...
  new_gdal_job(command_path = c("raster", "footprint"), arguments = merged_args, arg_mapping = .arg_mapping, update_intent = .update_intent)
}
