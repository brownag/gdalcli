...
#' @title convex-hull: Compute the convex hull of geometries of a vector dataset
#' @description
#' `gdal vector convex-hull` computes the smallest convex polygon that covers the input geometry. Generally the result will be a Polygon, but for degenerate inputs it may be a LineString or a Point. This command can also be used as a step of gdal_vector_pipeline.
#' 
#' See \url{https://gdal.org/en/release-3.13/programs/gdal_vector_convex-hull.html} for detailed GDAL documentation.
...
#' job <- gdal_vector_convex_hull(input = "countries.gpkg", 
#'     output = "countries_convexhull.gpkg")
#' \dontrun{
#'   result <- gdal_job_run(job)
#' }
#' @export
gdal_vector_convex_hull <- function(input,
  input_format = NULL,
  input_layer = NULL,
  output,
  output_format = NULL,
  ...
  if (!missing(input) && inherits(input, 'gdal_job')) {
    ...
    return(extend_gdal_pipeline(piped_job, c("vector", "convex-hull"), new_args))
  }


  merged_args <- .merge_alias_parameters(list(...), new_args)
  .update_intent_mapping <- NULL
  .update_intent <- infer_update_intent("gdal_vector_convex_hull", merged_args, .update_intent_mapping)
  ...
  new_gdal_job(command_path = c("vector", "convex-hull"), arguments = merged_args, arg_mapping = .arg_mapping, update_intent = .update_intent)
}
