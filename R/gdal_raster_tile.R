...
#' @description
#' `gdal raster tile` generates a directory with small tiles and metadata, following the OGC WMTS Specification. Simple web pages with viewers based on OpenLayers and Leaflet are generated as well - so anybody can comfortably explore your maps on-line and you do not need to install or configure any special software (like MapServer) and the map displays very fast in the web browser. You only need to upload the generated directory onto a web server. It generates PNG files by default, but other output formats can be selected (JPEG, WEBP, GTiff, etc.). Note that not all formats support non-Byte data types. JPEG and WEBP only support Byte. PNG supports Byte and UInt16. GeoTiff can support all data types. It can also use a tiling scheme fully adapted to the input raster, in terms of origin and resolution, when using the ``raster`` tiling scheme. In that scheme, tiles at the maximum zoom level will have the same resolution as the raster. Starting with GDAL 3.12, gdal raster tile can be used as the last step of a pipeline.
#' 
#' See \url{https://gdal.org/en/release-3.13/programs/gdal_raster_tile.html} for detailed GDAL documentation.
...
#' @param output_nodata Output nodata value
...
#' @param parallel_method Parallelization method (thread, spawn, fork). Choices: "thread", "spawn", "fork"
...
gdal_raster_tile <- function(input,
  ...
  if (!missing(parallel_method)) new_args[["parallel_method"]] <- parallel_method
  ...
  .update_intent <- infer_update_intent("gdal_raster_tile", merged_args, .update_intent_mapping)
  ...
  .arg_mapping <- list(
    ...
    parallel_method = list(min_count = 0, max_count = 1),
  ...
  new_gdal_job(command_path = c("raster", "tile"), arguments = merged_args, arg_mapping = .arg_mapping, update_intent = .update_intent)
}
