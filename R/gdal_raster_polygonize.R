...
#' @description
#' `gdal raster polygonize` creates vector polygons for all connected regions of pixels in the raster sharing a common pixel value. Each polygon is created with an attribute indicating the pixel value of that polygon. A mask (either explicit, or implicit through nodata value or band) associated to the selected band can be used to determine the which pixels should be included in the processing. The utility can create the output vector datasource if it does not already exist, otherwise it may append to an existing one. The utility is based on the ::cppGDALPolygonize function which has additional details on the algorithm. The following options are available:
#' 
