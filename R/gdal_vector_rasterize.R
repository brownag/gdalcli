...
#' @description
#' `gdal vector rasterize` burns vector geometries into a raster. By default (unless --update or --add) is specified, a new raster is created with a specified resolution (--resolution) or size (--size). Pixel values will either be set to a constant (the value 1, or some other value specified with --burn), the Z value of feature geometries (with --3d), or the value of a field specified with --attribute-name.
#' 
#' See \url{https://gdal.org/en/release-3.13/programs/gdal_vector_rasterize.html} for detailed GDAL documentation.
...
#' https://gdal.org/en/release-3.13/programs/gdal_vector_rasterize.html
...
gdal_vector_rasterize <- function(input,
  ...
  new_gdal_job(command_path = c("vector", "rasterize"), arguments = merged_args, arg_mapping = .arg_mapping, update_intent = .update_intent)
}
