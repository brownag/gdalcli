...
#' @param source_layer_field_name GDAL argument
#' @param source_layer_field_content GDAL argument
#' @param field_strategy GDAL argument
...
#' @examples
#' \dontrun{
#' gdal_vector_concat(...) |> gdal_run()
#' }

#' @export
gdal_vector_concat <- function(input_format = NULL,
  open_option = NULL,
  input,
  input_layer = NULL,
  output_format = NULL,
  ...
  if (!missing(source_layer_field_name)) args[["source_layer_field_name"]] <- source_layer_field_name
  if (!missing(source_layer_field_content)) args[["source_layer_field_content"]] <- source_layer_field_content
  if (!missing(field_strategy)) args[["field_strategy"]] <- field_strategy
  if (!missing(src_crs)) args[["src_crs"]] <- src_crs
  if (!missing(dst_crs)) args[["dst_crs"]] <- dst_crs

  new_gdal_job(command_path = c("gdal", "vector", "concat"), arguments = args)
}
