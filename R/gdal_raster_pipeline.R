...
#' 
#' See \url{https://gdal.org/en/3.11.4/programs/gdal_raster_pipeline.html} for detailed GDAL documentation.
#' @param jobs A vector of gdal_job objects to execute in sequence, or NULL to use pipeline string
#' @param input Input raster dataset (Dataset path). Format: `NA` (Default: `NA`)
...
#' @param pipeline Pipeline string. Format: `NA` (Default: `NA`) (ignored if jobs is provided)
...
gdal_raster_pipeline <- function(jobs = NULL,
  ...
  .arg_mapping <- list(
    ...
    pipeline = list(min_count = NA, max_count = NA),
