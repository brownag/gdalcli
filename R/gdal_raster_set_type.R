...
#' @description
#' `gdal raster set-type` can be used to force the output image bands to have a specific data type. Values may be truncated or rounded if the output data type is "smaller" than the input one. This subcommand is also available as a potential step of gdal_raster_pipeline
#' 
#' See \url{https://gdal.org/en/release-3.13/programs/gdal_raster_set-type.html} for detailed GDAL documentation.
...
#' @param output_data_type Output data type. Choices: "UInt8", "Int8", "UInt16", "Int16", "UInt32", ... (required)
...
gdal_raster_set_type <- function(input,
  ...
  if (!missing(input) && inherits(input, 'gdal_job')) {
    ...
    return(extend_gdal_pipeline(piped_job, c("raster", "set-type"), new_args))
  }
  ...
  new_gdal_job(command_path = c("raster", "set-type"), arguments = merged_args, arg_mapping = .arg_mapping, update_intent = .update_intent)
}
