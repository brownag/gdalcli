...
#' @title concave-hull: Compute the concave hull of geometries of a vector dataset
#' @description
#' `gdal vector concave-hull` computes the smallest concave polygon that covers the input geometry. Generally the result will be a Polygon, but for degenerate inputs it may be a LineString or a Point. It uses the :cppOGRGeometry::ConcaveHullOfPolygons method for polygonal input, and :cppOGRGeometry::ConcaveHull otherwise. This command can also be used as a step of gdal_vector_pipeline.
#' 
#' See \url{https://gdal.org/en/release-3.13/programs/gdal_vector_concave-hull.html} for detailed GDAL documentation.
...
#' @param ratio Ratio controlling the concavity (required). Range: (`0` to `1`)
...
#' @param allow_holes Allow holes in the output polygon (Logical) (Default: `false`)
#' @param tight Whether the hull must follow the outer boundaries of the input polygons (Logical) (Default: `false`)
...
#' @examples
#' \dontrun{
#' # TODO: No examples available for gdal_vector_concave_hull.
#' # See GDAL documentation at:
#' https://gdal.org/en/release-3.13/programs/gdal_vector_concave-hull.html
#' job <- gdal_vector_concave_hull()
#' # gdal_job_run(job)
#' }
#' @export
gdal_vector_concave_hull <- function(input,
  input_format = NULL,
  input_layer = NULL,
  output,
  output_format = NULL,
  ...
  if (!missing(ratio)) new_args[["ratio"]] <- ratio
  ...
  if (!missing(allow_holes)) new_args[["allow_holes"]] <- allow_holes
  if (!missing(tight)) new_args[["tight"]] <- tight
  ...
  if (!missing(input) && inherits(input, 'gdal_job')) {
    ...
    return(extend_gdal_pipeline(piped_job, c("vector", "concave-hull"), new_args))
  }


  merged_args <- .merge_alias_parameters(list(...), new_args)
  .update_intent_mapping <- NULL
  .update_intent <- infer_update_intent("gdal_vector_concave_hull", merged_args, .update_intent_mapping)
  ...
  .arg_mapping <- list(
    ...
    ratio = list(min_count = 0, max_count = 1),
    ...
    allow_holes = list(min_count = 0, max_count = 1),
    tight = list(min_count = 0, max_count = 1)
  )

  new_gdal_job(command_path = c("vector", "concave-hull"), arguments = merged_args, arg_mapping = .arg_mapping, update_intent = .update_intent)
}
