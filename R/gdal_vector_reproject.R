...
#' @description
#' `gdal vector reproject` can be used to reproject a vector dataset. The program can reproject to any supported projection. This command can also be used as a step of gdal_vector_pipeline.
#' 
#' See \url{https://gdal.org/en/release-3.13/programs/gdal_vector_reproject.html} for detailed GDAL documentation.
...
#' @param output_crs Output CRS (required)
...
#' job <- gdal_vector_reproject(input = "in.gpkg", output = "out.gpkg", overwrite = TRUE, 
...
gdal_vector_reproject <- function(input,
  ...
  .update_intent <- infer_update_intent("gdal_vector_reproject", merged_args, .update_intent_mapping)
  ...
  new_gdal_job(command_path = c("vector", "reproject"), arguments = merged_args, arg_mapping = .arg_mapping, update_intent = .update_intent)
}
