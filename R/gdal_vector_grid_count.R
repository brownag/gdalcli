...
#' 
#' See \url{https://gdal.org/en/stable/programs/gdal_vector_grid_count.html} for detailed GDAL documentation.
...
#' job <- gdal_vector_grid_count(input = "data.tif")
#' #
#' # Inspect the job (optional)
#' # print(job)

#' @export
gdal_vector_grid_count <- function(job = NULL,
  input = NULL,
  input_format = NULL,
  min_points = NULL,
  ...
  job_input <- handle_job_input(job, new_args, c("vector", "grid", "count"))
  if (job_input$should_extend) {
    # Extend pipeline from existing job
    return(extend_gdal_pipeline(job_input$job, c("vector", "grid", "count"), new_args))
  } else {
    # Create new job with merged arguments
    merged_args <- job_input$merged_args
  }

  new_gdal_job(command_path = c("vector", "grid", "count"), arguments = merged_args)
}
