...
#' @description
#' `gdal vector select` can be used to select a subset of fields. ``select`` can also be used as a step of gdal_vector_pipeline.
#' 
#' See \url{https://gdal.org/en/release-3.13/programs/gdal_vector_select.html} for detailed GDAL documentation.
...
#' @param fields Fields to select (or exclude if --exclude) (Character vector) (required)
...
#' https://gdal.org/en/release-3.13/programs/gdal_vector_select.html
...
gdal_vector_select <- function(input,
  ...
  .arg_mapping <- list(
    ...
    exclude = list(min_count = 0, max_count = 1),
    ignore_missing_fields = list(min_count = 0, max_count = 1)
  )

  new_gdal_job(command_path = c("vector", "select"), arguments = merged_args, arg_mapping = .arg_mapping, update_intent = .update_intent)
}
