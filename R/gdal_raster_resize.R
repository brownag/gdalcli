...
#' @description
#' `gdal raster resize` can be used to resize a raster dataset without changing the georeferenced extents. This subcommand is also available as a potential step of gdal_raster_pipeline
#' 
#' See \url{https://gdal.org/en/release-3.13/programs/gdal_raster_resize.html} for detailed GDAL documentation.
...
#' https://gdal.org/en/release-3.13/programs/gdal_raster_resize.html
...
gdal_raster_resize <- function(input,
  ...
  .update_intent <- infer_update_intent("gdal_raster_resize", merged_args, .update_intent_mapping)
  ...
  new_gdal_job(command_path = c("raster", "resize"), arguments = merged_args, arg_mapping = .arg_mapping, update_intent = .update_intent)
}
