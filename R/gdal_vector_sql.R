...
#' @description
#' `gdal vector sql` returns one or several layers evaluated from SQL statements. Starting with GDAL 3.12, when using --update, and without an output dataset specified, this can be used to execute statements that modify the input dataset, such as UPDATE, DELETE, etc.
#' 
#' See \url{https://gdal.org/en/release-3.13/programs/gdal_vector_sql.html} for detailed GDAL documentation.
...
#' @param output_layer Output layer name(s) (Character vector)
#' @param sql SQL statement(s) (Character vector). Format: `<statement>|@<filename>` (required)
...
#' https://gdal.org/en/release-3.13/programs/gdal_vector_sql.html
#' job <- gdal_vector_sql()
#' # gdal_job_run(job)
#' }
#' @export
gdal_vector_sql <- function(input,
  ...
  new_gdal_job(command_path = c("vector", "sql"), arguments = merged_args, arg_mapping = .arg_mapping, update_intent = .update_intent)
}
