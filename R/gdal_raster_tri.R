...
#' 
#' See \url{https://gdal.org/en/stable/programs/gdal_raster_tri.html} for detailed GDAL documentation.
...
#' job <- gdal_raster_tri(input = "data.tif")
#' #
#' # Inspect the job (optional)
#' # print(job)

#' @export
gdal_raster_tri <- function(job = NULL,
  input = NULL,
  input_format = NULL,
  output = NULL,
  output_format = NULL,
  ...
  job_input <- handle_job_input(job, new_args, c("raster", "tri"))
  if (job_input$should_extend) {
    # Extend pipeline from existing job
    return(extend_gdal_pipeline(job_input$job, c("raster", "tri"), new_args))
  } else {
    # Create new job with merged arguments
    merged_args <- job_input$merged_args
  }

  new_gdal_job(command_path = c("raster", "tri"), arguments = merged_args)
}
