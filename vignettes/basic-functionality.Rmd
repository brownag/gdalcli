...
```{r pipeline-example}
# Create a processing pipeline: convert -> scale -> compress
...
# Inspect the job spec before execution
str(pipeline_job)

# Execute would be: gdal_job_run(pipeline_job)
```
...
gdal_job_run(info_verbose)
gdal_job_run(info_quiet)
```
...
# Execute all jobs
lapply(batch_jobs, gdal_job_run)
```
