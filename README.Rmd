...
### Example 10: Pipeline with Configuration Options
...
`gdalcli` supports GDAL 3.11 and later, with additional features available in GDAL 3.12+.

### Minimum Requirements

- **GDAL**: >= 3.11 (CLI framework minimum)
- **R**: >= 4.1
- **gdalraster**: >= 2.2.0 (recommended for some features)
...
- **Configuration Options**: GDAL config option support
- **VSI Support**: Virtual file system integration
- **Shell Script Generation**: Export pipelines to executable bash/zsh

### GDAL 3.12 Features

Additional algorithms and capabilities available in GDAL 3.12+:

- **GDALG Format Write Support**: Write pipelines to GDALG format for persistence and sharing
- **Additional Raster Algorithms**: blend, compare, neighbors, nodata-to-alpha, pansharpen, proximity, rgb-to-palette, update, zonal-stats, as-features
- **Additional Vector Algorithms**: check-coverage, check-geometry, clean-coverage, index, layer-algebra, make-point, partition, set-field-type, simplify-coverage
- **GDALG Format Driver**: Native support for GDALG as a format driver with metadata
- **Pipeline Features**: Additional pipeline capabilities and algorithm metadata
